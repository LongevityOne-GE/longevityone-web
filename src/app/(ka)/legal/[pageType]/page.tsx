...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return generateLegalMetadata({ slug: pageType, lang: 'ka' })
}

export default async function KaLegalPage({ params }: Props): Promise<React.ReactElement> {
  const { pageType } = await params
  if (!isLegalSlug(pageType)) notFound()
  return <LegalPageLayout slug={pageType} lang="ka" />
}
